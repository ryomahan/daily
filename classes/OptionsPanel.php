...
class OptionsPanel
{
    ...
    protected $args = [];               // panel options
    protected $parent = '';             // parent menu slug
    protected $title = '';              // panel title
    protected $position = null;         // position in the menu order this item should appear
    protected $slug = '';               // panel slug
    protected $option_name = '';        // Option name to use for saving our options in the database.
    protected $option_group_name = '';  // Option group name.
    protected $user_capability = '';    // User capability allowed to access the options page.
    protected $settings = [];           // Our array of settings.

    public function __construct(array $args, array $settings)
    {
        $this->args              = $args;
        $this->settings          = $settings;
        $this->position          = $this->args['position'] ?? null;
...
?>
    ...
    ?>
        <input type="text" id="<?php echo esc_attr($args['label_for']); ?>" name="<?php echo $this->option_name; ?>[<?php echo esc_attr($args['label_for']); ?>]" value="<?php echo esc_attr($value); ?>">
        <?php if ($description) { ?>
    ...
    ?>
        <textarea type="text" id="<?php echo esc_attr($args['label_for']); ?>" rows="<?php echo esc_attr(absint($rows)); ?>" cols="<?php echo esc_attr(absint($cols)); ?>" name="<?php echo $this->option_name; ?>[<?php echo esc_attr($args['label_for']); ?>]"><?php echo esc_attr($value); ?></textarea>
        <?php if ($description) { ?>
            <p class="description"><?php echo esc_html($description); ?></p>
        <?php } ?>
    <?php
    ...
    public function render_checkbox_field($args)
    {
        ...
        $description = $this->settings[$option_name]['description'] ?? '';
    ?>
        <input type="checkbox" id="<?php echo esc_attr($args['label_for']); ?>" name="<?php echo $this->option_name; ?>[<?php echo esc_attr($args['label_for']); ?>]" <?php checked($value, 1, true); ?>>
        <?php if ($description) { ?>
            <p class="description"><?php echo esc_html($description); ?></p>
        <?php } ?>
    <?php
    ...
    ?>
        <select id="<?php echo esc_attr($args['label_for']); ?>" name="<?php echo $this->option_name; ?>[<?php echo esc_attr($args['label_for']); ?>]">
